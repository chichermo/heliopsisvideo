...
console.log('🚀 INSERTANDO TODOS LOS TOKENS ORIGINALES EN BASE DE DATOS...\n');

// Conectar a la base de datos
const dbPath = path.join(__dirname, 'access_tokens.db');
...
console.log(`📊 Total de tokens originales: ${originalTokens.length}\n`);

// Función para insertar todos los tokens
function insertAllOriginalTokens() {
...
async function insertTokens() {
    try {
        const result = await insertAllOriginalTokens();
        
        console.log(`\n📈 RESUMEN:`);
        console.log(`   ✅ Tokens insertados exitosamente: ${result.successCount}/${originalTokens.length}`);
        console.log(`   ❌ Errores: ${result.errorCount}`);
        
        if (result.successCount > 0) {
            console.log(`\n🎉 ¡${result.successCount} tokens originales insertados!`);
