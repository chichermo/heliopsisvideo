...
console.log('🚀 RECUPERANDO TODOS LOS TOKENS PERDIDOS EN RENDER...\n');
...
// Función para insertar un token en Render
function insertTokenToRender(tokenData, index) {
    return new Promise((resolve, reject) => {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (response.success) {
                        console.log(`✅ Token ${index + 1}/${lostTokens.length} insertado: ${tokenData.email}`);
                        resolve(response);
                    } else {
                        console.log(`❌ Error insertando token ${index + 1}: ${response.error}`);
                        reject(new Error(response.error));
                    }
                } catch (error) {
                    console.log(`❌ Error parseando respuesta ${index + 1}: ${error.message}`);
                    reject(error);
                }
            });
        });
        ...
        req.on('error', (error) => {
            console.log(`❌ Error de conexión ${index + 1}: ${error.message}`);
            reject(error);
        });
...
// Función para insertar todos los tokens
async function insertAllTokens() {
    let successCount = 0;
    let errorCount = 0;

    for (let i = 0; i < lostTokens.length; i++) {
        try {
            await insertTokenToRender(lostTokens[i], i);
            successCount++;
            
            // Pequeña pausa entre requests para no sobrecargar Render
            if (i < lostTokens.length - 1) {
                await new Promise(resolve => setTimeout(resolve, 500));
            }
        } catch (error) {
            errorCount++;
            console.log(`❌ Falló token ${i + 1}: ${lostTokens[i].email}`);
        }
    }

    console.log(`\n📈 RESUMEN:`);
    console.log(`   ✅ Tokens insertados exitosamente: ${successCount}`);
    console.log(`   ❌ Tokens con error: ${errorCount}`);
    console.log(`   📊 Total procesados: ${lostTokens.length}`);
