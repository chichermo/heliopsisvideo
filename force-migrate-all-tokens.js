...
// Script para forzar la migración y inserción de todos los tokens
const dbPath = path.join(__dirname, 'database', 'access_tokens.db');

console.log('🔄 FORZANDO MIGRACIÓN DE TOKENS...');
...
const db = new sqlite3.Database(dbPath, (err) => {
    ...
    // Eliminar tabla existente y crear una nueva con todos los tokens
    ...
    db.run('DROP TABLE IF EXISTS simple_tokens', (err) => {
        ...
        console.log('✅ Tabla eliminada, creando nueva...');
...
function insertAllTokens() {
    ...
    console.log(`📊 Insertando ${allTokens.length} tokens...`);
    ...
    allTokens.forEach((tokenData, index) => {
        ...
        db.run(insertSQL, [
            ...
            if (err) {
                ...
                console.log(`✅ Token insertado: ${tokenData.email} (${index + 1}/${allTokens.length})`);
                insertedCount++;
            }
            
            // Verificar si hemos procesado todos los tokens
            if (processedCount === allTokens.length) {
                console.log('\n📊 RESUMEN FINAL:');
                console.log(`✅ Tokens insertados exitosamente: ${insertedCount}`);
                console.log(`❌ Errores: ${processedCount - insertedCount}`);
